...
10. Forward lessons from the Arize teardown (July 16) — submission doctrine
10a. The judge-visible Codex delta is part of the success criterion. A working Mirror proves the Design criterion; it does not prove Technological Implementation, which is scored on Codex use. The submission (README + video) must make the before/after delta explicit and visible: v1 was built on a no-code platform over weeks with no tests and no evals; v2 was rebuilt with Codex in days with real auth, persistence, and a passing anti-sycophancy eval harness. Evidence to surface, not summarize: the timestamped commit history, the Day-1 deployment log (25+ deployments to first green), the /feedback Codex session, and the eval harness passing 6/6 the same day the prompt it tests was validated. The demo must contain at least one explicit "Codex enabled this speed/quality" beat, with receipts on screen.
10b. Conform to the judged shape (standing doctrine). When organizer signals — judging criteria, FAQs, livestreams, required artifacts — indicate what shape of submission is rewarded, conform the submission to that shape rather than building the preferred shape and hoping rigor carries it. Applied here: Codex workflow and GPT-5.6 usage must be foregrounded in the video voiceover and README exactly as the rules require (where Codex accelerated, where key decisions were made), even at the cost of time spent showcasing architecture we are proud of. Architecture that judges can't see or don't score is not a differentiator. (The July 13 livestream gate in earlier planning was honored before commit; this section generalizes it.)
